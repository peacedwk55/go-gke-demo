...
- **Write-back credential (the manifest-bump commit needs one — the default `GITHUB_TOKEN` triggers no re-run and may be blocked by branch protection):** use a **GitHub App** installation token (preferred) or a fine-grained PAT with `contents:write` scoped to this repo only, stored as a repo secret. Branch protection on `main` stays ON; the bot goes in the protection rule's **bypass list** (or pushes to a `release/bump` branch with auto-merge if bypass is not allowed by org policy). Document the chosen path in the workflow README.
  ...
  - This is a consequence of one person building the whole thing, not a design position, and it leaves a real hole: the Trivy gate, the twelve invariants and the "no kubectl in CI" rule all live in `.github/workflows/` and `scripts/`, so whoever can push can also switch them off in a one-line diff that looks small.
...
Flow ของการเปลี่ยน infra ทีละขั้น

┌─ ขั้น 0 ─────────────────────────────────────────────── คุณ ──┐
│  แก้ไฟล์ .tf ใน infra/terraform/  →  branch ใหม่  →  push     │
└───────────────────────────────────┬───────────────────────────┘
                                    ↓ เปิด PR
┌─ ขั้น 1 ── workflow terraform.yaml วิ่งเอง ─────── GitHub ──┐
│                                                              │
│  Job A: fmt-validate    ★ ไม่ต้องมี credential เลย ★         │
│    1. terraform fmt -check -recursive     จับ format         │
│    2. terraform init -backend=false       ไม่ต่อ GCS         │
│    3. terraform validate                  จับ syntax/type    │
│                                                              │
│  Job B: plan            ★ ต้องมี credential ★                │
│    1. auth เข้า GCP ด้วย Workload Identity Federation        │
│       (แลก OIDC token — ไม่มีไฟล์ key)                       │
│    2. terraform init      ต่อ GCS backend จริง               │
│    3. terraform plan      อ่าน state จริง เทียบกับโค้ด       │
│    4. โพสต์ plan เป็น comment ใน PR                          │
│    5. plan fail → job fail                                   │
└───────────────────────────────────┬──────────────────────────┘
                                    ↓
┌─ ขั้น 2 ─────────────────── ★ คนอ่าน plan ★ ─────── คุณ ──┐
│  มองหาคำเหล่านี้:                                          │
│     must be replaced   /  forces replacement  /  to destroy │
└───────────────────────────────────┬────────────────────────┘
                                    ↓ merge
┌─ ขั้น 3 ─── merge เข้า main ──────────────────────────────┐
│  โค้ดเข้า main แล้ว  แต่ ★ GCP ยังไม่เปลี่ยนอะไรเลย ★     │
└───────────────────────────────────┬───────────────────────┘
                                    ↓
┌─ ขั้น 4 ────────────── คน apply เอง ──────────────── คุณ ──┐
│  terraform plan "-var-file=demo.tfvars" "-out=tfplan"      │
│  terraform apply tfplan                                    │
└───────────────────────────────────┬───────────────────────┘
                                    ↓
┌─ ขั้น 5 ─── Terraform ล็อก state → เปลี่ยน GCP → เขียน state กลับ GCS ─┐
└───────────────────────────────────┬───────────────────────────────────┘
                                    ↓ (เฉพาะกรณีที่ output เปลี่ยน)
┌─ ขั้น 6 ─── ส่งข้อเท็จจริงให้ชั้นแอป ─────────────────────┐
│  terraform output image_repository  →  kustomization.yaml  │
│  terraform output ksa_annotation    →  ServiceAccount      │
│  แล้ว commit → จากนั้น ArgoCD ทำต่อตามเส้นของแอป           │
└────────────────────────────────────────────────────────────┘
...
kubectl -n observability get pvc                 # รอจนว่าง
gcloud compute disks list --filter="-users:*"    # ต้องไม่เหลืออะไร
